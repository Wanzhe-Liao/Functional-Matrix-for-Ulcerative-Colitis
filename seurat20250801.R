...
for(dir in all_dirs) {
  if(file.exists(file.path(dir, "barcodes.tsv.gz")) && 
     file.exists(file.path(dir, "features.tsv.gz")) && 
     file.exists(file.path(dir, "matrix.mtx.gz"))) {
    dirs_sample <- c(dirs_sample, dir)
  }
}

names(dirs_sample) <- gsub(".+[/\\\\](.+)", "\\1", dirs_sample)
cat("Found", length(dirs_sample), "valid 10X directories:\n")
print(names(dirs_sample))
...
#######################Enhanced Fig 1A - Epithelial Cell Quality Violin Plot#######################
# 为审稿人意见创建增强的Fig 1A violin plot，显示上皮细胞类型和数据集分层
epithelial_cell_types <- c("Enterocytes", "Basal Progenitor Cells", "Goblet Cells", 
                          "Epithelial Cells", "Inflammatory Epithelial Cells", "Enteroendocrine Cells")

# 过滤上皮细胞
pbmc_epi <- subset(pbmc, subset = celltype %in% epithelial_cell_types)

# 添加样本组信息 (UC vs Control)
pbmc_epi@meta.data$group <- ifelse(grepl("^UC", pbmc_epi@meta.data$orig.ident), "UC", "Control")

# 创建增强的Fig 1A - 上皮细胞质量指标violin plot
pdf(file="Figure1A_enhanced_epithelial_violin.pdf", width=16, height=10)
p1 <- VlnPlot(pbmc_epi, 
...
for(i in 1:length(p1)) {
  p1[[i]] <- p1[[i]] + 
    ...
    scale_fill_manual(values = c("Control" = "#4CAF50", "UC" = "#F44336"))
}

# 组合图形
library(patchwork)
combined_plot <- wrap_plots(p1, ncol = 3)
combined_plot <- combined_plot + plot_annotation(
  title = "Enhanced Fig 1A: Data Quality Metrics by Epithelial Cell Type and Dataset",
  subtitle = "Violin plots showing nFeature_RNA, nCount_RNA, and percent.mt distribution across epithelial cell types",
...
# 保存PNG版本用于手稿
ggsave("Figure1A_enhanced_epithelial_violin.png", combined_plot, width = 16, height = 10, dpi = 300)

# 备选方案：使用ggplot2创建更详细的分面图
library(reshape2)
meta_epi <- pbmc_epi@meta.data
quality_data <- meta_epi[, c("celltype", "group", "nFeature_RNA", "nCount_RNA", "percent.mt", "orig.ident")]
quality_melted <- melt(quality_data, 
                      id.vars = c("celltype", "group", "orig.ident"), 
                      variable.name = "metric", 
                      value.name = "value")

# 创建按样本分面的详细图
pdf(file="Figure1A_detailed_by_sample.pdf", width = 18, height = 12)
p_detailed <- ggplot(quality_melted, aes(x = celltype, y = value, fill = group)) +
  geom_violin(trim = FALSE, scale = "width", alpha = 0.7) +
  geom_boxplot(width = 0.1, position = position_dodge(0.9), fill = "white", alpha = 0.8) +
  facet_grid(metric ~ orig.ident, scales = "free_y") +
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 10),
        strip.text = element_text(size = 11),
        legend.position = "top") +
  labs(title = "Quality Metrics by Epithelial Cell Type, Dataset Group, and Individual Sample",
       x = "Epithelial Cell Type",
       y = "Value",
       fill = "Dataset Group") +
  scale_fill_manual(values = c("Control" = "#4CAF50", "UC" = "#F44336"))
print(p_detailed)
...
ggsave("Figure1A_detailed_by_sample.png", p_detailed, width = 18, height = 12, dpi = 300)

# 输出上皮细胞统计信息
cat("Enhanced Fig 1A generated successfully!\n")
cat("Epithelial cell types included:", length(epithelial_cell_types), "types\n")
cat("Total epithelial cells:", ncol(pbmc_epi), "cells\n")
cat("UC samples:", sum(pbmc_epi@meta.data$group == "UC"), "cells\n")
cat("Control samples:", sum(pbmc_epi@meta.data$group == "Control"), "cells\n")

# 清理临时对象
rm(pbmc_epi, quality_data, quality_melted, meta_epi, p1, combined_plot, p_detailed)
...
# pbmc_t_Bcell <- subset(pbmc, subset=group=="Treat_B-cells")  # Skip this subset as 'group' is not defined
...
# Skip B cell analysis since pbmc_t_Bcell is not defined
# All B cell analysis code is commented out due to undefined variable